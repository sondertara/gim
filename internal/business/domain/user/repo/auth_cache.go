...
func (*authCache) Get(userId, deviceId int64) (*entity.Device, error) {
...
func (*authCache) Set(userId, deviceId int64, device entity.Device) error {
...
func (*authCache) GetAll(userId int64) (map[int64]entity.Device, error) {
	result, err := db.RedisCli.HGetAll(AuthKey + strconv.FormatInt(userId, 10)).Result()
	if err != nil {
		return nil, gerrors.WrapError(err)
	}

	var devices = make(map[int64]entity.Device, len(result))
