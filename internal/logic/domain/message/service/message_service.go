...
func (*messageService) Add(ctx context.Context, message entity.Message) error {
	return repo.MessageRepo.Save(message)
}
...
func (*messageService) Sync(ctx context.Context, userId, seq int64) (*pb.SyncResp, error) {
	...
	pbMessages := entity.MessagesToPB(messages)
...
func (*messageService) ListByUserIdAndSeq(ctx context.Context, userId, seq int64) ([]entity.Message, bool, error) {
	var err error
	if seq == 0 {
...
func (*messageService) SendToUser(ctx context.Context, sender *pb.Sender, toUserId int64, req *pb.SendMessageReq) (int64, error) {
	...
	if req.IsPersist {
		...
		selfMessage := entity.Message{
			...
			ToUserIds:    entity.FormatUserIds(req.ToUserIds),
